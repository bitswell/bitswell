...
Your task is the body of the ASSIGNED commit on your branch:
...
```bash
git log -1 --format='%(trailers:key=Scope,valueonly)' HEAD
```

## Context Window Budget

Your budget is in the ASSIGNED commit's `Budget` trailer. Reserve at least 10% for your final commit. At 90% consumption, commit with `Task-Status: BLOCKED` and `Blocked-Reason: resource_limit`, then exit cleanly.
